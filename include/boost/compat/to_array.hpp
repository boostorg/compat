...
constexpr std::array<remove_cvref_t<T>, N> to_array_lvalue(T (&a)[N], index_sequence<I...>)
{
    return {{a[I]...}};
}

template <class T, std::size_t N, std::size_t... I>
constexpr std::array<remove_cvref_t<T>, N> to_array_rvalue(T (&&a)[N], index_sequence<I...>)
{
    return {{std::move(a[I])...}};
}
